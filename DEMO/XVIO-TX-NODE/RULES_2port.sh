...
#  echo "in_port=22,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1001" >> /tmp/flows.txt
  echo "in_port=23,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1001" >> /tmp/flows.txt
done
...
for match_val in $(seq 1 16383); do
  ...
  echo "in_port=1001,tun_id=$match_val,actions=22" >> /tmp/flows.txt
done

for match_val in $(seq 16384 32767); do
  echo "in_port=1000,tun_id=$match_val,actions=23" >> /tmp/flows.txt
  echo "in_port=1001,tun_id=$match_val,actions=23" >> /tmp/flows.txt
done
