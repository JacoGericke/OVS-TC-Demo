...
for match_val in $(seq 1 32767); do
  #ovs-ofctl add-flow ${bridge} in_port=13,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1001
  ...
  #ovs-ofctl add-flow ${bridge} in_port=12,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1001
done

#for match_val in $(seq 16384 32767); do
#  ovs-ofctl add-flow ${bridge} in_port=13,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1000
#  ovs-ofctl add-flow ${bridge} in_port=12,tcp,tcp_dst=$match_val,actions=set_tunnel:$match_val,output:1000
#done
...
for match_val in $(seq 16384 32767); do
  #ovs-ofctl add-flow ${bridge} in_port=1001,tun_id=$match_val,actions=13
